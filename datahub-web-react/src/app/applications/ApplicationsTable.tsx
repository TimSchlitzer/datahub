...
const ApplicationsTable = ({ searchQuery, searchData, loading: propLoading, networkStatus, refetch }: Props) => {
    ...
    const showDeleteConfirmation = useCallback(
        (applicationUrn: string) => {
            ...
            if (!applicationData) {
                message.error(t('applications.findError'));
                return;
            }
    ...
    const handleDeleteApplication = useCallback(() => {
        deleteApplicationMutation({
            ...
            .catch((e: any) => {
                message.error(`${t('applications.deleteFailed')}: ${e.message}`);
            });
    ...
    const columns = useMemo(
        () => [
            {
                title: t('applications.application'),
                key: 'application',
                render: (record) => {
        ...
        [entityRegistry, searchQuery, showDeleteConfirmation, t],
    );
    ...
    return (
        <>
            ...
            <ConfirmationModal
                ...
                modalTitle={t('applications.deleteConfirmTitle')}
                modalText={t('applications.deleteConfirmContent', { name: applicationDisplayName })}
                closeButtonText={t('common.cancel')}
                confirmButtonText={t('applications.delete')}
            />
        </>
    );
};
