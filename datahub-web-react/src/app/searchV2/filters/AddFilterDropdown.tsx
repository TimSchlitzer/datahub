...
export default function AddFilterDropdown({ fields = DEFAULT_FILTER_FIELDS, onAddFilter, includeCount }: Props) {
    ...
    const translatedFields = useTranslatedFilters(fields);

    const items = translatedFields.map((field) => {
        return {
            key: field.field,
            label: (
                <FilterPopover
...
function FilterPopover({ field, onAddFilter, setDropdownOpen, includeCount }: PopoverProps) {
    ...
    const icon =
        ...
        (field.type === FieldType.ENTITY &&
            ...
            entityRegistry.getIcon(field.entityTypes[0], 12, IconStyleType.ACCENT, theme.colors.textSecondary));
