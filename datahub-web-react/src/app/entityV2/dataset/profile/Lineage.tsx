        ...
        <>
            <div>
                <ViewRawButtonContainer>
                    <Button onClick={() => navigateToLineageUrl({ location, history, isLineageMode: true })}>
                        {t('entity.dataset.lineage.viewGraph')}
                    </Button>
                </ViewRawButtonContainer>
            </div>
            <Space direction="vertical" style={{ width: '100%' }} size="large">
                <List
                    ...
                    header={<Typography.Title level={3}>{t('entity.dataset.lineage.upstream')}</Typography.Title>}
                    renderItem={(item) => (
                ...
                <List
                    ...
                    header={<Typography.Title level={3}>{t('entity.dataset.lineage.downstream')}</Typography.Title>}
                    renderItem={(item) => (
                        <List.Item style={{ paddingTop: '20px' }}>
                            {entityRegistry.renderPreview(item?.type || EntityType.Dataset, PreviewType.PREVIEW, item)}
                        </List.Item>
