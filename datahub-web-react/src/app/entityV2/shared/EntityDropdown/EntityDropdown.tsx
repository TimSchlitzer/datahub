...
const EntityDropdown = (props: Props) => {
    ...
    if (menuItems.has(EntityMenuItems.COPY_URL) && navigator.clipboard) {
        menuItemsList.push({
            type: 'item' as const,
            key: '0',
            title: t('entityDropdown.copyUrl'),
            icon: 'Link',
            onClick: () => {
                navigator.clipboard.writeText(pageUrl);
                message.info(t('entityDropdown.copiedUrl'), 1.2);
            },
        });
    }
    ...
    if (menuItems.has(EntityMenuItems.ANNOUNCE)) {
        menuItemsList.push({
            type: 'item' as const,
            key: '1-1',
            title: t('entityDropdown.addNote'),
    ...
    if (menuItems.has(EntityMenuItems.ADD_TERM)) {
        menuItemsList.push({
            type: 'item' as const,
            key: '2',
            title: t('entityDropdown.addTerm'),
            icon: 'Plus',
    ...
    if (menuItems.has(EntityMenuItems.ADD_TERM_GROUP)) {
        menuItemsList.push({
            type: 'item' as const,
            key: '3',
            title: t('entityDropdown.addTermGroup'),
            icon: 'FolderPlus',
    ...
    if (menuItems.has(EntityMenuItems.RAISE_INCIDENT)) {
        menuItemsList.push({
            type: 'item' as const,
            key: '6',
            title: t('entityDropdown.raiseIncident'),
            icon: 'Warning',
    ...
    if (
        ...
        menuItemsList.push({
            type: 'item' as const,
            key: 'link',
            title: t('entityDropdown.linkNewerVersion'),
            icon: 'Link',
    ...
    if (onEntityProfile && entityData?.versionProperties?.isLatest) {
        menuItemsList.push({
            type: 'item' as const,
            key: 'unlink',
            title: t('entityDropdown.unlinkVersion'),
            icon: 'LinkBreak',
    ...
    if (onEntityProfile && entityData?.versionProperties && setDrawer) {
        menuItemsList.push({
            type: 'item' as const,
            key: 'showVersions',
            title: t('entityDropdown.showVersions'),
            icon: 'GitCommit',
            onClick: () => {
                analytics.event({
    ...
    if (menuItems.has(EntityMenuItems.SHARE)) {
        ...
        if (navigator.clipboard) {
            shareChildren.push({
                type: 'item' as const,
                key: 'copy-link',
                title: t('entityDropdown.copyLink'),
                icon: 'Link',
                onClick: () => {
        ...
        if (navigator.clipboard) {
            shareChildren.push({
                type: 'item' as const,
                key: 'copy-urn',
                title: t('entityDropdown.copyUrn'),
                icon: 'Copy',
                onClick: () => {
        ...
        if (navigator.clipboard) {
            ...
            shareChildren.push({
                type: 'item' as const,
                key: 'copy-name',
                title: t('entityDropdown.copyName'),
                icon: 'Copy',
                onClick: () => {
            ...
            onClick: () => {
                ...
                const link = qs.stringifyUrl({
                    ...
                    query: {
                        ...
                        body: t('entityDropdown.checkOut', { type: displayType, url: linkText }),
                    },
                });
        ...
        menuItemsList.push({
            ...
            tooltip: shouldDisplayChildDeletionWarning(entityType, entityData, me.platformPrivileges)
                ? t('entityDropdown.cantDeleteWithEntities', {
                      type: entityRegistryV2.getEntityName(entityType),
